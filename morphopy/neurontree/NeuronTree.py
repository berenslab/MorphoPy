...
class NeuronTree:
    ...
    def __init__(self, swc=None, scaling=1.0, node_data=[], edge_data=[], graph=None):
        """
        ...
        if graph:
            ...
            if swc is not None:
                ...
                if type(swc) == pd.DataFrame:
                    ...
                    pos = (
                        np.array([swc["x"].values, swc["y"].values, swc["z"].values]).T
                        / scaling
                    )
                ...
                node_data = list(
                    zip(
                        n,
                        [
                            dict(zip(node_keys, [pos[ix], t[ix], radius[ix]]))
                ...
                # create edge data
                parent_idx = np.array(
                    [np.where(n == k)[0][0] if k != -1 else -1 for k in pid]
                )

                # calculate euclidean distance between end points of edge
                ec = np.sqrt(
                    np.sum(
                        (pos[parent_idx[parent_idx != -1]] - pos[parent_idx != -1])
                        ** 2,
                        axis=1,
                    )
                )
                ...
                edge_data = list(
                    zip(
                        ...
                        n[node_idx],
                        [
                            dict(zip(edge_keys, [ec[ix], ec[ix]]))
                            for ix in range(ec.shape[0])
                        ],
                    )
                )
    ...
    def _remove_redundant_nodes(self):
        ...
        nodeindices = (
            np.array(list(self.get_edge_attributes("euclidean_dist").values())) == 0
        )
        edgelist = list(
            np.array(list(self.get_edge_attributes("euclidean_dist").keys()))[
                nodeindices
            ]
        )
        while edgelist:
    ...
    def _make_tree(self):
        ...
        roots = self.get_root(
            return_all=True
        )  # in case there are disconnected neurites
    ...
    def get_topological_minor(self):
        ...
        node_data_new = [
            (node, node_data[node]) for node in np.append(other_points, tips)
        ]
        ...
        while nodes:
            ...
            if current_node != root:
                ...
                ec = np.sqrt(
                    np.sum(
                        (node_data[current_node]["pos"] - node_data[pred]["pos"]) ** 2
                    )
                )
                ...
                edge_data_new.append(
                    (
                        pred,
                        current_node,
                        dict(euclidean_dist=ec, path_length=path_length),
                    )
                )
    ...
    def smooth_neurites(self, dim=1, window_size=21):
        ...
        for c in to_visit:
            ...
            if len(path) > window_size:
                data = np.array(path)
                datahat = savgol_filter(
                    data[:, dim], window_size, 3, mode="nearest"
                ).reshape(-1, 1)
        ...
        nx.set_edge_attributes(
            G, name="euclidean_dist", values=dict(zip(G.edges(), e_attr))
        )
        nx.set_edge_attributes(
            G, name="path_length", values=dict(zip(G.edges(), e_attr))
        )
    ...
    def get_path_length(self, weight="euclidean_dist"):
        ...
        pl = nx.single_source_dijkstra_path_length(
            self.get_graph(), source=self.get_root(), weight=weight
        )
        return pl
    ...
    def truncate_nodes(self, perc=0.1, no_trunc_nodes=None):
        """
        ...
        if type_ix is None:
            ...
            if type(type_ix) == list:
                if self._nxversion >= 2:
                    ...
                    nodes = [
                        k for k in self._G.nodes() if self._G.node[k]["type"] in type_ix
                    ]
            else:
                if self._nxversion >= 2:
                    # changed for version 2.x of networkX
                    ...
                    nodes = [
                        k for k in self._G.nodes() if self._G.node[k]["type"] == type_ix
                    ]
    ...
    def edges(self, start=None, type_ix=None, data=False):
        ...
        if type_ix is None:
            ...
            edges = [
                x
                for x in self._G.edges(start, data=data)
    ...
    def get_axon_edges(self, start=None, data=False):
        ...
        return [
            x
            for x in self._G.edges(start, data=data)
            if (x[0] in axon_nodes or x[1] in axon_nodes)
        ]
    ...
    def get_dendrite_edges(self, start=None, data=False):
        ...
        return [
            x
            for x in self._G.edges(start, data=data)
            if (x[0] in dendrite_nodes or x[1] in dendrite_nodes)
        ]
    ...
    def get_branchpoints(self):
        ...
        # is this correct???:
        bp_indx = np.where(
            np.array(np.sum(nx.adjacency_matrix(self._G).todense(), axis=1)).flatten()
            > 1
        )[0]
    ...
    def get_strahler_order(self):
        ...
        for a in active:
            a = int(a)
            so, counts = np.unique(
                [strahler_order[s] for s in self._G.successors(a)], return_counts=True
            )
    ...
    def _get_distribution_data(
        self, key="branch_order", dist_measure=None, angle_type="axis"
    ):
        """
    ...
    def get_histogram(
        self, key="branch_order", dist_measure=None, angle_type="axis", **kwargs
    ):
        """
    ...
    def get_kde_distribution(self, key, dist_measure=None, angle_type="axis", **kwargs):
        ...
        if (
            key.find("angle") > -1
        ):  # if we are operating on angles then set max to be 180 degree
            if dim > 1:
                max_[1] = 180
            else:
                max_ = 180
    ...
    def get_soma_angles(self):
        ...
        for succ in successors:
            if self._nxversion >= 2:
                # changed for version 2.x of networkX
                v = (
                    self.get_graph().nodes[succ]["pos"]
                    - self.get_graph().nodes[r]["pos"]
                )
            else:
        ...
        for bp in branchpoints:
            ...
            for succ in successors:  # create a vector for each branching edge
                if self._nxversion >= 2:
                    ...
                    v = (
                        self.get_graph().nodes[succ]["pos"]
                        - self.get_graph().nodes[bp]["pos"]
                    )
                else:
                    v = (
                        self.get_graph().node[succ]["pos"]
                        - self.get_graph().node[bp]["pos"]
                    )
            ...
            for u, v in combinations(branches, 2):
                # compute the angles between all possible combinations. Only keep the len(branches) - 1 smallest of them
                all_angles.update(
                    {(bp, u[0], v[0]): angle_between(u[1], v[1]) * 180 / np.pi}
                )
    ...
    def get_sholl_intersection_profile(
        self, proj="xy", steps=36, centroid="centroid", radii=None
    ):
        """
    ...
    def _resample_tree_data(self, d=1):
        ...
        while active_nodes:
            ...
            new_swc = pd.concat([new_swc, new_entry])
            ...
            for p, s, data in out_edges:
                ...
                while s not in branchpoints and s not in tips:
                    if dist < d:
                        ...
                        s_x, s_y, s_z, s_rad, s_type = sample_new_points(
                            pred, s, data["path_length"], dist, d
                        )
                        ...
                        new_swc = pd.concat([new_swc, new_entries])
    ...
    def draw_3D(
        ...
        axon = go.Scatter3d(
            x=X_a, y=Y_a, z=Z_a, mode="lines", line=axon_line, name="Axon"
        )

        dendrites = go.Scatter3d(
            x=X_d, y=Y_d, z=Z_d, mode="lines", line=dendrite_line, name="Dendrites"
        )
        apical_dendrites = go.Scatter3d(
            x=X_ad,
            y=Y_ad,
            z=Z_ad,
            mode="lines",
        ...
        axis = dict(
            showbackground=False, showgrid=False, showticklabels=False, title=""
        )
    ...
    def to_swc(self):
        ...
        df = pd.DataFrame(
            data=d, columns=["n", "type", "x", "y", "z", "radius", "parent"]
        )
